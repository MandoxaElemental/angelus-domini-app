...
export default function RegisterScreen({
  ...
  const handleRegister = async () => {
    ...
    try {
      setLoading(true);
      const data = await register(email, username, password, selectedCountry.name);
      if (data.session) {
        goToHome();
      } else {
      ...
      <ImageBackground source={churchBg} style={styles.bg} resizeMode="cover">
        ...
        >
          ...
          <View style={styles.formSection}>
            ...
            {/* Username */}
            <TouchableOpacity onPress={() => openField("username")} activeOpacity={0.8}>
              <GlassInput>
                <Text style={[styles.textInput, username ? styles.filledText : styles.placeholderText]}>
                  {username || "Username"}
                </Text>
              </GlassInput>
            </TouchableOpacity>
            ...
            {/* Email */}
            <TouchableOpacity onPress={() => openField("email")} activeOpacity={0.8}>
              <GlassInput>
                <Text style={[styles.textInput, email ? styles.filledText : styles.placeholderText]}>
                  {email || "Email"}
                </Text>
              </GlassInput>
            </TouchableOpacity>
            ...
            {/* Password */}
            <TouchableOpacity onPress={() => openField("password")} activeOpacity={0.8}>
              <GlassInput style={{ flexDirection: "row", alignItems: "center" }}>
                <Text style={[styles.textInput, { flex: 1 }, password ? styles.filledText : styles.placeholderText]}>
                  {password ? "•".repeat(Math.min(password.length, 20)) : "Password"}
                </Text>
                <Ionicons name="lock-closed-outline" size={18} color="rgba(255,230,167,0.5)" />
              </GlassInput>
            </TouchableOpacity>
              ...
              <TouchableOpacity activeOpacity={1} onPress={() => {}}>
                <View style={styles.floatSheet}>
                  ...
                  <View style={styles.floatHeader}>
                    <TouchableOpacity onPress={cancelField} style={styles.floatHeaderBtn}>
                      <Text style={styles.floatCancelText}>Cancel</Text>
                    </TouchableOpacity>

                    <Text style={styles.floatTitle}>{getFieldLabel()}</Text>

                    <TouchableOpacity onPress={confirmField} style={[styles.floatHeaderBtn, { alignItems: "flex-end" }]}>
                  ...
                  <View style={styles.floatInputRow}>
                    <TextInput
                      ...
                      secureTextEntry={activeField === "password" && !showTempPassword}
                    ...
                    {activeField === "password" && (
                      ...
                      >
                        <Ionicons
                          name={showTempPassword ? "eye-outline" : "eye-off-outline"}
                          size={24}
                          color="#C8922A"
                        />
                      </TouchableOpacity>
                  ...
                  {activeField === "password" && (
                    <Text style={styles.floatHelperText}>
                      {showTempPassword ? "Password is visible" : "Password is hidden"}
                    </Text>
                  )}

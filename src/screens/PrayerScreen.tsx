...
export default function PrayerScreen() {
  ...
  const getCurrentPrayerTime = () => {
    ...
    if (currentMinutes >= 6 * 60 && currentMinutes < 12 * 60) return "6am";
    if (currentMinutes >= 12 * 60 && currentMinutes < 18 * 60) return "12pm";
    return "6pm";
  };
  ...
  const playBellSequence = async (count: number) => {
    for (let i = 0; i < count; i++) {
      ...
      Animated.parallel([
        Animated.timing(ringScale, { toValue: 1.35, duration: 1400, easing: Easing.out(Easing.ease), useNativeDriver: false }),
        Animated.timing(ringOpacity, { toValue: 0, duration: 1400, useNativeDriver: false }),
      ]).start();
  ...
  useEffect(() => {
    const updatePrayerTime = () => setSelectedTime(getCurrentPrayerTime());
    updatePrayerTime();
  ...
  const renderPrayer = () => {
    if (item.type === "versicle") {
      return <Animated.Text style={[styles.versicle, { opacity: fadeAnim }]}>{item.text}</Animated.Text>;
    }
    if (item.type === "response") {
      return <Animated.Text style={[styles.responseItalic, { opacity: fadeAnim }]}>{item.text}</Animated.Text>;
    }
    if (item.type === "prayer") {
      return <Animated.Text style={[styles.prayer, { opacity: fadeAnim }]}>{item.text}</Animated.Text>;
    }
  };
    ...
    <>
      <View style={styles.container}>
        ...
        <View style={styles.timeSelector}>
          {[{ label: "6:00 AM", value: "6am" }, { label: "12:00 PM", value: "12pm" }, { label: "6:00 PM", value: "6pm" }].map((t) => {
        ...
        <View style={styles.card}>
          {item.type === "prayer" ? (
            <View style={styles.prayerScrollWindow}>
              ...
              <LinearGradient colors={["#FFFAF2", "transparent"]} style={styles.topFade} pointerEvents="none" />
              <LinearGradient colors={["transparent", "#FFFAF2"]} style={styles.bottomFade} pointerEvents="none" />
            </View>
          ) : item.type === "bell" ? (
        ...
        >
          ...
          >
            <Text style={styles.buttonText}>
              {autoPlay ? "Praying..." : currentStep === PRAYER_SEQUENCE.length - 1 ? "Finish" : "Continue"}
            </Text>
          </LinearGradient>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  topFade: { position: "absolute", top: 0, left: 0, right: 0, height: 40, zIndex: 2 },
  bottomFade: { position: "absolute", bottom: 0, left: 0, right: 0, height: 40, zIndex: 2 },
  card: {
    margin: 24, padding: 24, backgroundColor: "#FFFAF2", borderRadius: 24,
    maxHeight: 260, minHeight: 260, justifyContent: "center",
    shadowColor: "#4a392f", shadowOffset: { width: 0, height: 12 },
